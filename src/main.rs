...
impl ws::Handler for Client {
    fn on_open(&mut self, _: ws::Handshake) -> ws::Result<()> {
        ...
        info!(
            target: LOG_TARGET,
            "Registering player with message: {:?}",
            msg
        );
        self.out.send(msg)
    }
    ...
    fn on_message(&mut self, msg: ws::Message) -> ws::Result<()> {
        if let ws::Message::Text(text) = msg {
            ...
            match route_result {
                Err(e) => error!(
                    target: LOG_TARGET,
                    "Got error \'{:?}\' when routing message: {}",
                    e,
                    text
                ),
            ...
            warn!(
                target: LOG_TARGET,
                "Unexpectedly received non-string message: {:?}",
                msg
            )
        }
...
fn start_websocket_thread(
    ...
    thread::spawn(move || {
        ...
        let result = ws::connect(connection_url, |out| Client {
            out: Arc::from(out),
            snake: snake::Snake,
            config: config.clone(),
    ...
    loop {
        ...
        if let Err(e) = rec {
            if e == mpsc::TryRecvError::Disconnected {
                debug!(
                    target: LOG_TARGET,
                    "Stopping heartbeat due to channel disconnecting"
                );
                break;
            }
        } else {
            debug!(
                target: LOG_TARGET,
                "Stopping heartbeat due to finished execution"
            );
            break;
        }

        debug!(target: LOG_TARGET, "Sending heartbeat request");
        let send_result = out.send(render_outbound_message(Outbound::HeartBeat {
            receivingPlayerId: id.clone(),
        }));
        if let Err(e) = send_result {
            error!(
                target: LOG_TARGET,
                "Unable to send heartbeat, got error {:?}",
                e
            );
        }
    }
...
fn start_heart_beat_thread(
    ...
    thread::spawn(move || {
        ...
        if let Ok((id, out)) = res {
            ...
            error!(
                target: LOG_TARGET,
                "Unable to start heart beat, the channel has been closed."
            );
        };
    })
}
...
fn start_client() {
    ...
    debug!(
        target: LOG_TARGET,
        "Joining Websocket thread gave result {:?}",
        websocket_res
    );
    ...
    if let Err(e) = send_res {
        error!(
            target: LOG_TARGET,
            "Unable to send done message, got error {:?}",
            e
        );
    }
    ...
    debug!(
        target: LOG_TARGET,
        "Joining heartbeat thread gave result {:?}",
        heartbeat_res
    );
}
